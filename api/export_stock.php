...
switch ($action) {
    ...
    case 'create':
        ...
        try {
            $lock = $conn->prepare("SELECT id, name, stock_quantity FROM products WHERE id = ? AND is_active = 1 FOR UPDATE");
            ...
            if ($locked_product['stock_quantity'] < $quantity) {
                ...
                echo json_encode([
                    'success' => false,
                    'message' => "Không đủ hàng để xuất. Hiện còn {$locked_product['stock_quantity']} sản phẩm."
                ]);
                exit;
            }

            $receipt_id = 'PX_' . date('YmdHis') . '_' . random_int(1000, 9999);

            $header = $conn->prepare("INSERT INTO export_receipts (id, created_by, created_at) VALUES (?, ?, NOW())");
            $header->bind_param("si", $receipt_id, $created_by);
            $header->execute();
            $header->close();

            $detail = $conn->prepare("INSERT INTO export_receipt_details (receipt_id, product_id, quantity, notes) VALUES (?, ?, ?, ?)");
            $detail->bind_param("siis", $receipt_id, $product_id, $quantity, $notes);
            $detail->execute();
            $detail->close();

            $update = $conn->prepare("UPDATE products SET stock_quantity = stock_quantity - ? WHERE id = ? AND stock_quantity >= ?");
            ...
            echo json_encode([
                'success'       => true,
                'message'       => "Xuất kho thành công. Sản phẩm \"{$locked_product['name']}\": -{$quantity} → {$new_qty} sản phẩm.",
    ...
    case 'create_batch':
        ...
        try {
            ...
            $detail_stmt = $conn->prepare("INSERT INTO export_receipt_details (receipt_id, product_id, quantity, notes) VALUES (?, ?, ?, ?)");
            $lock = $conn->prepare("SELECT id, name, stock_quantity FROM products WHERE id = ? AND is_active = 1 FOR UPDATE");
            $update = $conn->prepare("UPDATE products SET stock_quantity = stock_quantity - ? WHERE id = ? AND stock_quantity >= ?");
            ...
            foreach ($items as $item) {
                ...
                if ($locked_product['stock_quantity'] < $quantity) {
                    throw new Exception("Sản phẩm \"{$locked_product['name']}\" không đủ số lượng (tồn kho: {$locked_product['stock_quantity']}, yêu cầu: {$quantity}).");
                }
    ...
    case 'detail':
        ...
        echo json_encode(getStockDetail($conn, 'export', $receipt_id));
        break;
