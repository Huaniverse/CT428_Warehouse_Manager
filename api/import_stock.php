...
switch ($action) {
    ...
    case 'create':
        ...
        try {
            $lock = $conn->prepare("SELECT id, name FROM sanpham WHERE id = ? AND is_active = 1 FOR UPDATE");
            ...
            $code = 'PN_' . date('YmdHis') . '_' . random_int(1000, 9999);

            $header = $conn->prepare("INSERT INTO phieu_nhap (code, created_by, created_at) VALUES (?, ?, NOW())");
            $header->bind_param("si", $code, $created_by);
            $header->execute();
            $header->close();

            $detail = $conn->prepare("INSERT INTO chi_tiet_phieu_nhap (receipt_code, product_id, quantity, note) VALUES (?, ?, ?, ?)");
            $detail->bind_param("siis", $code, $product_id, $quantity, $note);
            $detail->execute();
            $detail->close();

            $update = $conn->prepare("UPDATE sanpham SET stock = stock + ? WHERE id = ?");
            ...
            echo json_encode([
                'success'       => true,
                'message'       => "Nhập kho thành công. Sản phẩm \"{$locked_product['name']}\": +{$quantity} → {$new_stock} sản phẩm.",
                'code'          => $code,
                'new_stock'     => $new_stock
            ]);
        } catch (Exception $e) {
    ...
    case 'create_batch':
        ...
        try {
            ...
            $detail_stmt = $conn->prepare("INSERT INTO chi_tiet_phieu_nhap (receipt_code, product_id, quantity, note) VALUES (?, ?, ?, ?)");
            $lock = $conn->prepare("SELECT id, name FROM sanpham WHERE id = ? AND is_active = 1 FOR UPDATE");
            $update = $conn->prepare("UPDATE sanpham SET stock = stock + ? WHERE id = ?");
    ...
    case 'detail':
        ...
        echo json_encode(getStockDetail($conn, 'import', $code));
        break;
