...
switch ($action) {
    ...
    case 'create':
        ...
        try {
            $stmt = $conn->prepare(
                "INSERT INTO phieu_nhap (san_pham, so_luong, ghi_chu, nguoi_tao, ngay_tao) VALUES (?, ?, ?, ?, NOW())"
            );
            ...
            echo json_encode([
                'success'       => true,
                'message'       => "Nhập kho thành công. Sản phẩm \"{$product['TenSP']}\": +{$so_luong} → {$new_qty} sản phẩm.",
                'ma_phieu'      => $ma_phieu,
    ...
    case 'list':
        ...
        // Lấy dữ liệu
        $sql = "SELECT pn.ma_phieu, pn.so_luong, pn.ghi_chu, pn.ngay_tao,
                       sp.MaSP, sp.TenSP,
