document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btnSubmit) btnSubmit.addEventListener('click', function () {
        ...
        apiFetch(BASE + '/admin/api/users.php?action=create', { method: 'POST', body: fd })
            .then(data => {
...
function loadUsers() {
    ...
    apiFetch(BASE + '/admin/api/users.php?action=list')
        .then(data => {
            ...
            tbody.innerHTML = data.users.map(u => {
                ...
                const canManage = isAdmin ? u.role !== 'admin' : u.role === 'staff';
                const actions = isSelf
                    ...
                    + '</div>';
                return '<tr><td><div class="user_info_cell"><div class="user_avatar">' + initials + '</div><div><div class="u_fullname">' + safeName + '</div><div class="u_username">@' + safeUsername + '</div></div></div></td>'
...
function toggleUser(id, newStatus) {
    ...
    const BASE = window.BASE_URL || '';
    apiFetch(BASE + '/admin/api/users.php?action=toggle', { method: 'POST', body: fd })
...
function deleteUser(id, name) {
    ...
    fd.append('id', id);
    const BASE = window.BASE_URL || '';
    apiFetch(BASE + '/admin/api/users.php?action=delete', { method: 'POST', body: fd })
        .then(data => { showToast(data.message, data.success ? 'success' : 'error'); if (data.success) loadUsers(); })
        .catch(err => showToast(err.message || 'Lỗi kết nối máy chủ.', 'error'));
}
...
function loadSessions() {
    ...
    const BASE = window.BASE_URL || '';
    apiFetch(BASE + '/admin/api/users.php?action=sessions')
        .then(data => {
            ...
            list.innerHTML = data.sessions.map(s => {
                ...
                return '<div class="session_item ' + (s.is_current ? 'current_session' : '') + '">'
                    ...
                    + '<div class="session_user">' + safeName + ' (@' + safeUsername + ') ' + currentTag
                    + ' <span class="user_role_badge ' + s.role + '" style="margin-left:6px;">' + roleLabel + '</span></div>'
...
function kickUser(userId) {
    ...
    const BASE = window.BASE_URL || '';
    apiFetch(BASE + '/admin/api/users.php?action=kick', { method: 'POST', body: fd })
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (permModal) {
        ...
        document.getElementById('btnSubmitPermissions')?.addEventListener('click', function () {
            ...
            const BASE = window.BASE_URL || '';
            apiFetch(BASE + '/admin/api/users.php?action=update_permissions', { method: 'POST', body: fd })
                .then(data => {
                    this.disabled = false;
